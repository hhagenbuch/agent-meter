...
/** OpenTelemetry SDK exporting OTLP/gRPC to the collector; agent-meter instruments from here. */
@Configuration
public class OtelConfig {
    ...
    public OpenTelemetry openTelemetry(@Value("${otel.exporter.otlp.endpoint:http://localhost:4317}") String endpoint) {
        Resource resource = Resource.getDefault().toBuilder()
                .put(AttributeKey.stringKey("service.name"), "agent-meter-demo-agent")
                .build();
